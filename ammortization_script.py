...
def loan_amortization_sheet(row):
    ...
    amortization_term_months = row["amortization_term_months"]  # Number of months for the amortization
    mortgage_term_months = row["mortgage_term_months"]  # Mortgage term (should be same as renewal_period)
    ...
    def generate_payment_dates(start_date_str, frequency):
        ...
        start_date = start_date_str#datetime.strptime(start_date_str, "%Y-%m-%d")
        ...
        for _ in range(freq_details["periods_per_year"]*((row["amortization_term_months"]//12)+1)):
            ...
            if freq_details["months_offset"] > 0:
                current_date += timedelta(days=30 * freq_details["months_offset"])
            else:
                current_date += timedelta(days=freq_details["day_offset"])
    ...
    dates=generate_payment_dates(start_date_str=start_date, frequency=payment_frequency)
    ...
    df_amortization["date"]=pd.to_datetime(df_amortization["date"]).dt.strftime('%d-%m-%Y')
    ...
    filename="individual amortization tables/LoanTape "+ str(loanTape)+".xlsx"
    # Create a Pandas Excel writer using a context manager
    with pd.ExcelWriter(filename, engine='openpyxl') as writer:
        # Write each DataFrame to a different sheet
        df_amortization.round(2).to_excel(writer, sheet_name='Periodic Amortization Table', index=False)
        daily_amortization_df.round(2).to_excel(writer, sheet_name='Daily Amortization Table')
        df_monthly.round(2).to_excel(writer, sheet_name='Monthly Amortization Table', index=False)

    
    print("Ammortion tables for LoanTape "+ str(loanTape) + " have been written to '" + filename+"'")
...
if "__main__":      
    ...
    for i in range(len(loan_data)):
        daily_amortization.append(pd.read_excel("individual amortization tables/LoanTape "+ str(i)+".xlsx", sheet_name='Daily Amortization Table'))
        monthly_amortization.append(pd.read_excel("individual amortization tables/LoanTape "+ str(i)+".xlsx", sheet_name='Monthly Amortization Table'))
    ...
    consolidated_daily_df = consolidated_daily_df.groupby('date').sum().reset_index()
    # Convert the 'date' column to datetime format
    consolidated_daily_df['date'] = pd.to_datetime(consolidated_daily_df['date'], format='%d-%m-%Y')
    ...
    #consolidated_daily_df.to_excel("daily test.xlsx")
    consolidated_monthly_df = consolidated_monthly_df.groupby('date').sum().reset_index()
    # Convert the 'date' column to datetime format
    consolidated_monthly_df['date'] = pd.to_datetime(consolidated_monthly_df['date'], format='%d-%m-%Y')

    # Sort the dataframe by the 'date' column
    consolidated_monthly_df = consolidated_monthly_df.sort_values(by='date')
    consolidated_daily_df["date"]=consolidated_daily_df["date"].dt.strftime('%d-%m-%Y')
    consolidated_monthly_df["date"]=consolidated_monthly_df["date"].dt.strftime('%d-%m-%Y')
    consolidated_daily_df=consolidated_daily_df.reset_index()
    #consolidated_daily_df.rename(columns={'index': 'day'}, inplace=True)
    consolidated_daily_df.drop(columns=["Unnamed: 0", "period", "index"], inplace=True)
    ...
    with pd.ExcelWriter(filename, engine='openpyxl') as writer:
        # Write each DataFrame to a different sheet
        consolidated_daily_df.round(2).to_excel(writer, sheet_name='Consolidated Daily Table')
        consolidated_monthly_df.round(2).to_excel(writer, sheet_name='Consolidated Monthly Table', index=False)
